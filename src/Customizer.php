...
/**
 * Defines the 'customizer' service.
 */
class Customizer extends DefaultPluginManager implements CustomizerInterface {
  ...
  public function updateStylesheetForTheme($theme = NULL): void {
    $theme = $theme ?? $this->activeTheme->getName();
    $values = $this->configFactory->get('customizer.theme.' . $theme)
      ->get('values');
    ...
    // Save value to file.
    $this->fileSystem->saveData($css, "public://customizer.$theme.css", FileSystemInterface::EXISTS_REPLACE);
  ...
  public function deleteStylesheetForTheme($theme = NULL): void {
    ...
    $this->fileSystem->delete("public://customizer.$theme.css");
